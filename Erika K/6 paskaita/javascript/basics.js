...
function multiply(manoElementas, indeksas) {
  console.log('Mane iskviete su kiekvienu elementu, ir dabar jis yra:', manoElementas, indeksas);
}

function mulAllNumbers(numberArray) {
  let result = 1;
  // numberArray.forEach(num => result *= num);
  for (let i = 0; i < numberArray.length; i++) {
...
function stundentsAverage(studArray) {
  let ageTotal = 0;
  for (let i = 0; i < studArray.length; i++) {
    ageTotal += studArray[i].age;
  }
  return ageTotal / studArray.length;
  return studArray
    .map(stud => stud.age)
    .reduce((res, stud) => res += stud / studArray.length, 0);
}
